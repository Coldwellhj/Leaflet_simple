...
public class LogisticsInformationView extends View {
     ...
     * 绘制界面参数
     * */
    List<ListActivityBean.LogisticsData> logisticsDataList;
    ...
    private void init() {
        ...
        mPaintPhone.setColor(getResources().getColor(R.color.blue));
        mPaintPhone.setTextSize(35.0F);

        textPaintPhone= new TextPaint();
        textPaintPhone.setColor(getResources().getColor(R.color.blue));
     ...
     * */
    public void setLogisticsDataList( List<ListActivityBean.LogisticsData> logisticsDataList){
        ...
        requestLayout();//强制重绘（强行进入onDraw方法）
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < logisticsDataList.size(); i++) {
            ...
            if (i == 0) {
                ...
                canvas.drawText(((ListActivityBean.LogisticsData)data.get(i)).getTime()+"", left * 2 + radius * 2 + 10, heightList.get(i) + interval  , mPaint);
                //文字换行
                TextPaint textPaint = new TextPaint();
...
//                LogUtils.i("获取最后一行占的宽度"+layout.getLineWidth(layout.getLineCount()-1));
                 ...
                 * 分割带电话的数据
                 * */
                String[] splitData=splitString(((ListActivityBean.LogisticsData)data.get(i)).getContext()+"");
                if(splitData!=null){
                    //说明有电话号码
                    splitPhoneData(splitData,canvas,textPaint,0,true);
                }else{
                    //说明没有电话号码，直接用StaticLayout绘制文本
                    StaticLayout layout = new StaticLayout(((ListActivityBean.LogisticsData)data.get(i)).getContext()+"", textPaint, (int) (windowWidth * 0.8), Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);
               ...
               //画文字
                ...
                canvas.drawText(((ListActivityBean.LogisticsData)data.get(i)).getTime()+"", left * 2 + radius * 2 + 10,interval+heightList.get(i)+ heightData+top, paint);
                //文字换行
                TextPaint textPaint = new TextPaint();
